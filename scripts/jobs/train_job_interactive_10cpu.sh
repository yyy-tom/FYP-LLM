...
# Usage: srun -p gpu_24h --qos gpu --account gpu --gres=gpu:1 --cpus-per-task=10 --constraint=highcpucount --nodelist=gpu54 --pty bash scripts/jobs/train_job_interactive_10cpu.sh
...
echo "  sbatch scripts/jobs/train_job_gpu.sh"
echo ""
...
echo ""
echo "To train, run:"
echo "  uv run python scripts/training/train_qwen_counsel.py --config configs/config_7b_cpu.json"
echo ""
